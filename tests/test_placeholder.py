...
class TestTextUtils:
    ...
    def test_extract_bill_year(self) -> None:
        ...
        # Priority 1: Introduction Date wins
        assert (
            extract_bill_year(
                title="The Bill of 2020",
                introduction_date="15-02-2024",
                metadata={"year": 2022},
                url="https://prsindia.org/bill/2021",
            )
            == 2024
        )

        # Date object introduction date
        assert (
            extract_bill_year(introduction_date=date(2025, 6, 1), metadata={"year": 2022}) == 2025
        )

        # Priority 2: Metadata wins over URL and Title
        assert (
            extract_bill_year(
                title="The Bill of 2020",
                metadata={"year": 2022},
        ...
        # Metadata with pub_date containing year
        assert (
            extract_bill_year(
                metadata={"pub_date": "10-10-2023"}, url="https://prsindia.org/bill/2021"
            )
            == 2023
        )

        # Priority 3: URL wins over Title
        assert (
            extract_bill_year(title="The Bill of 2020", url="https://prsindia.org/bill/2021")
            == 2021
        )

        # Priority 4: Title is used as fallback
        assert extract_bill_year(title="The Bill of 2020") == 2020

        # Priority 5: NULL / None returned when no year is found
        assert (
            extract_bill_year(
                title="No year here",
                introduction_date="No year",
                metadata={},
                url="https://prsindia.org/bill/no-year",
            )
            is None
        )

        # Verify that it never invents/defaults to the current year
        assert extract_bill_year() is None
